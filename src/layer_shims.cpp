...
XRAPI_ATTR XrResult XRAPI_CALL thisLayer_xrSuggestInteractionProfileBindings(XrInstance instance,
                                                                             const XrInteractionProfileSuggestedBinding *binding) {
    static PFN_xrSuggestInteractionProfileBindings nextLayer_xrSuggestInteractionProfileBindings = GetNextLayerFunction(
...
XRAPI_ATTR XrResult XRAPI_CALL thisLayer_xrCreateActionSet(XrInstance instance, const XrActionSetCreateInfo *createInfo,
    ...
    stringstream buffer;
    buffer << "RNR xrCreateActionSet " << createInfo->actionSetName << " " << createInfo->localizedActionSetName << " "
           << *actionSet;
    Log::Write(Log::Level::Info, buffer.str());
...
XRAPI_ATTR XrResult
XRAPI_CALL thisLayer_xrCreateAction(XrActionSet actionSet, const XrActionCreateInfo *createInfo, XrAction *action) {
    ...
    stringstream buffer;
    buffer << "RNR xrCreateAction " << createInfo->actionName << " " << createInfo->localizedActionName << " "
           << *action;
    Log::Write(Log::Level::Info, buffer.str());
...
XRAPI_ATTR XrResult
XRAPI_CALL thisLayer_xrCreateActionSpace(XrSession session, const XrActionSpaceCreateInfo *createInfo, XrSpace *space) {
...
/// </summary>
XRAPI_ATTR XrResult
XRAPI_CALL thisLayer_xrCreateReferenceSpace(XrSession session, const XrReferenceSpaceCreateInfo *createInfo,
                                            XrSpace *space) {
...
//    }
    ...
    buffer << "RNR xrCreateReferenceSpace type " << name << " posef " << p.orientation.w << " " << p.orientation.x
           << " " << p.orientation.y << " " << p.orientation.z << " " << p.position.x << " " << p.position.y << " "
           << p.position.z << " space " << *space;
    Log::Write(Log::Level::Info, buffer.str());
...
XRAPI_ATTR XrResult
XRAPI_CALL recordLocateSpace(XrSpace space, XrSpace baseSpace, XrTime time, XrSpaceLocation *location) {
...
XRAPI_ATTR XrResult
XRAPI_CALL thisLayer_xrLocateSpace(XrSpace space, XrSpace baseSpace, XrTime time, XrSpaceLocation *location) {
    static PFN_xrLocateSpace nextLayer_xrLocateSpace = GetNextLayerFunction(xrLocateSpace);
...
bool replayLocateViews(XrSession session, const XrViewLocateInfo *viewlocateInfo, XrViewState *viewState,
    ...
    viewState->viewStateFlags = XR_VIEW_STATE_ORIENTATION_TRACKED_BIT | XR_VIEW_STATE_ORIENTATION_VALID_BIT |
...
XRAPI_ATTR XrResult
XRAPI_CALL recordLocateViews(XrSession session, const XrViewLocateInfo *viewlocateInfo, XrViewState *viewState,
                             uint32_t viewCapacityInput, uint32_t *viewCountOutput, XrView *views) {
    ...
    const auto result = nextLayer_xrLocateViews(session, viewlocateInfo, viewState, viewCapacityInput, viewCountOutput,
                                                views);
...
/// </summary>
XRAPI_ATTR XrResult
XRAPI_CALL thisLayer_xrLocateViews(XrSession session, const XrViewLocateInfo *viewlocateInfo, XrViewState *viewState,
                                   uint32_t viewCapacityInput, uint32_t *viewCountOutput, XrView *views) {
    static PFN_xrLocateViews nextLayer_xrLocateViews = GetNextLayerFunction(xrLocateViews);
...
XRAPI_ATTR XrResult XRAPI_CALL thisLayer_xrGetActionStateFloat(XrSession session, const XrActionStateGetInfo *getInfo,
...
XRAPI_ATTR XrResult
XRAPI_CALL thisLayer_xrGetActionStateVector2f(XrSession session, const XrActionStateGetInfo *getInfo,
                                              XrActionStateVector2f *state) {
    static PFN_xrGetActionStateVector2f nextLayer_xrGetActionStateFloat = GetNextLayerFunction(
...
XRAPI_ATTR XrResult XRAPI_CALL thisLayer_xrGetActionStateBoolean(XrSession session, const XrActionStateGetInfo *getInfo,
                                                                 XrActionStateBoolean *state) {
    static PFN_xrGetActionStateBoolean nextLayer_xrGetActionStateBoolean = GetNextLayerFunction(
...
std::vector<OpenXRLayer::ShimFunction> ListShims() {
    ...
    functions.emplace_back("xrSuggestInteractionProfileBindings",
                           PFN_xrVoidFunction(thisLayer_xrSuggestInteractionProfileBindings));
